"""
Apache Impala connector using pyhive with Kerberos support.
Impala uses HiveServer2 protocol.
"""
...
class ImpalaConnector(BaseConnector):
    """Apache Impala database connector using pyhive (HiveServer2 protocol)."""
    ...
    async def connect(self) -> None:
        ...
        if not isinstance(config, KerberosSourceConfig):
            if not hasattr(config, "host"):
                raise ValueError("Invalid Impala configuration: host is required")
        ...
        try:
            ...
            logger.info(f"Connected to Impala: {self.source_id}")
    ...
    def _create_connection(self) -> Any:
        """Create Impala connection (synchronous, called via to_thread)."""
        config = self._config

        host = getattr(config, "host", "localhost")
        port = getattr(config, "port", None) or DEFAULT_PORTS["impala"]
        ...
        if isinstance(config, KerberosSourceConfig):
            if config.auth_mechanism == "KERBEROS":
                auth = "KERBEROS"
                kerberos_service_name = config.principal or "impala"
        ...
        # Use hive.connect with Impala-specific settings
        # Note: For production Impala, you might want to use impyla instead
        return hive.connect(
            host=host,
            port=port,
            database=database,
    ...
    async def disconnect(self) -> None:
        try:
            ...
            logger.warning(f"Error during Impala disconnect: {e}")
        finally:
            self._connection = None
            self._kerberos_auth = None
            self._is_connected = False
            logger.info(f"Disconnected from Impala: {self.source_id}")
    ...
    async def search_objects(
        ...
        # Search schemas (databases in Impala)
        ...
        if opts.object_type == "column" and opts.table:
            ...
            for row in result.rows:
                col_name = row.get("name") or list(row.values())[0]
                data_type = row.get("type") or (list(row.values())[1] if len(row) > 1 else None)
